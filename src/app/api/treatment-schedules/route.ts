...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      console.error('Error fetching treatment schedules:', error.message)
    ...
    console.error('Unexpected error in GET /api/treatment-schedules:', error)
    return NextResponse.json(
...
// POST is handled by admin/doctor - creating schedule
export async function POST(request: Request) {
  try {
    ...
    console.error('Unexpected error in POST /api/treatment-schedules:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
